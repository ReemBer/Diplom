    ...
    def __call__(self, tensor_in, condition=None, training=None, slope=None):
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            with tf.variable_scope('pitch_time_private'):
                s1 = [tconv_layer(h, 32, (1, 1, 12), (1, 1, 12)) for _ in range(self.n_tracks)]  # 4, 16, 84
                s1 = [tconv_layer(s1[i], 16, (1, 3, 1), (1, 3, 1)) for i in range(self.n_tracks)]  # 4, 48, 84

            # Time-pitch private network
            with tf.variable_scope('time_pitch_private'):
                s2 = [tconv_layer(h, 32, (1, 3, 1), (1, 3, 1)) for _ in range(self.n_tracks)]  # 4, 48, 7
                s2 = [tconv_layer(s2[i], 16, (1, 1, 12), (1, 1, 12)) for i in range(self.n_tracks)]  # 4, 48, 84

            h = [tf.concat((s1[i], s2[i]), -1) for i in range(self.n_tracks)]

            # Merged private network
            with tf.variable_scope('merged_private'):
                h = [norm(tconv3d(h[i], 1, (1, 1, 1), (1, 1, 1))) for i in range(self.n_tracks)] # 4, 48, 84
                h = tf.concat(h, -1)
