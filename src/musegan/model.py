    ...
    def get_predict_nodes(self, z=None, y=None, c=None, params=None,
        ...
        with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
            ...
            nodes = {
                'z': z,
                # Get slope tensor (for straight-through estimators)
                'slope': tf.get_variable('slope', [], tf.float32, tf.constant_initializer(1.0), trainable=False)
            }
            ...
            if config['collect_save_arrays_op']:
                ...
                for key, value in arrays.items():
                    save_array_ops.append(tf.py_func(lambda array, suffix, k=key: _save_array(array, suffix, k), [
                        value, config['suffix']], tf.int32))
                    make_sure_path_exists(os.path.join(config['result_dir'], 'arrays', key))
